...
// カーソルの可視性（0.0 〜 1.0）
let cursorVisibility = 1.0;
let lastVolume = 1.0; // 前回の音量を記録
...
// 音声ファイルを事前に読み込む
function preload() {
  soundYuan = loadSound('assets/yuyaan.wav');
  soundYuyon = loadSound('assets/yuyoon.wav');
  soundYuyayuyon = loadSound('assets/yuyayuyon.wav');
}
...
function draw() {
  ...
  if (elapsedTime < 3000) {
    cursorVisibility = 1.0;
  } else if (elapsedTime < 4000) {
    // 3秒から4秒の間で1.0から0.0に減少
    cursorVisibility = map(elapsedTime, 3000, 4000, 1.0, 0.0);
  } else {
    cursorVisibility = 0.0;
  }

  // 音声の音量をカーソルの可視性に合わせる
  // 変更があったときだけ音量を設定するようにして負荷を減らす
  if (abs(cursorVisibility - lastVolume) > 0.01) {
    soundYuan.setVolume(cursorVisibility);
    soundYuyon.setVolume(cursorVisibility);
    soundYuyayuyon.setVolume(cursorVisibility);
    lastVolume = cursorVisibility;
  }
  ...
  if (cursorVisibility > 0) {
    ...
    for (let r = maxRadius; r > 0; r--) {
      ...
      fill(0, alpha * cursorVisibility); // 黒色＋透明度（可視性を乗算）
      ellipse(mouseX, mouseY, r * 2, r * 2);
    }
    fill(0, 255 * cursorVisibility); // 中心に小さな黒い点を描画
...
class Character {
  ...
  update() {
    ...
    if (dist < repulsionRadius) {
      ...
      // カーソルの可視性（消失具合）に合わせて斥力を弱める
      forceMagnitude *= cursorVisibility;
      ...
      if (this.isSpecial) {
        // 音声再生のためのヘルパー関数
        const triggerSound = (snd) => {
          // ブラウザの制限などでAudioContextが停止している場合は再開を試みる
        ...
        if (this.charIndex >= 0 && this.charIndex <= 3) {
          triggerSound(soundYuan);
        }
        // "ゆよーん" (index 4-7)
        else if (this.charIndex >= 4 && this.charIndex <= 7) {
          triggerSound(soundYuyon);
        }
        // "ゆやゆよん" (index 8-12)
        else if (this.charIndex >= 8 && this.charIndex <= 12) {
          triggerSound(soundYuyayuyon);
        }
      }
